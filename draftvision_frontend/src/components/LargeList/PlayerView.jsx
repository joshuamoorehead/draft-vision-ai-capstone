...
import { fetch2024Players, fetchPlayerStats } from '../../services/api';
    ...
    const loadPlayer = async () => {
      ...
      try {
        ...
        const processedPlayers = players.map(player => {
          // Extract xAV from the nested structure
          const xAV = player.db_predictions_2024?.[0]?.xAV;
          return {
            ...player,
            predictions: {
              xAV: xAV || parseFloat((11.31 / (player.draft_round + 0.5) + 1.51).toFixed(2))
            }
          };
        });
        ...
        const foundPlayer = processedPlayers.find(p => p.id === parseInt(playerId));
        if (!foundPlayer) {
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="bg-gray-800 bg-opacity-70 p-6 rounded-2xl shadow-2xl border border-indigo-500 border-opacity-30">
            ...
            <PlayerCard
              player={player}
              stats={stats}
              players={allPlayers} // Pass all players for comparison
            />
          </div>
        </div>
      </div>
